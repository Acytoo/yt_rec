...
#I will use cv2 cause there isn't that fucking foder
            ...
            for filename in os.listdir(subject_path):
                try:
                    ...
                    if (image_size is not None):
                        im = cv2.resize(im, image_size)    #Are you kidding me? Use cv2.resize to do this _acytoo
...
class App(object):
    ...
    def run(self):
        while True:
            ...
            img = cv2.resize(frame, (int(frame.shape[1]/2), int(frame.shape[0]/2)), interpolation = int(cv2.INTER_CUBIC))
            imgout = img.copy()
            for i,r in enumerate(self.detector.detect(img)):
                ...
                face = cv2.resize(face, self.model.image_size, interpolation = int(cv2.INTER_CUBIC))
...
if __name__ == '__main__':
    ...
    # parameter -t (or --traint) to learn the model from a given dataset.
